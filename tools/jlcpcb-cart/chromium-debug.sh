...
env DISPLAY="${DISPLAY:-:0}" WAYLAND_DISPLAY="${WAYLAND_DISPLAY:-wayland-0}" XDG_RUNTIME_DIR="/run/user/$(id -u)" \
    ...
    --new-window 'https://jlcpcb.com/user-center/smtPrivateLibrary/partsCart/' >/dev/null 2>&1 &
for _ in $(seq 1 30); do curl -s "http://127.0.0.1:${PORT}/json/version" >/dev/null && { echo "chromium listening on :${PORT}, log in to jlcpcb.com in the window"; exit 0; }; sleep 1; done
