#!/usr/bin/env node
// Debug helper: log every request to HOST (default jlcpcb.com) from EVERY tab (method, url, body) so the
// add-to-cart call can be identified.  node dump_requests.js [seconds] [host]
const puppeteer = require("puppeteer-core");
const HOST = process.argv[3] || "jlcpcb.com";
(async () => {
  ...
  const attach = async page => {
    ...
    page.on('request', req => {
      const u = req.url(); if (!["xhr","fetch"].includes(req.resourceType()) || !u.includes(HOST) || /\.(js|css|png|jpg|svg|woff2?|gif|ico)(\?|$)/.test(u)) return;
  ...
  console.log(`logging all ${HOST} requests from all tabs...`);
